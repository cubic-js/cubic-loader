...
class Auth {
  ...
  async verify(id, config) {
    ...
    this.verifyKeys(id, config)
    await this.verifyUser(id, config)
  }

  /**
   * Assign RSA keys to api or auth node configs
   */
  verifyKeys(id, config) {
    if (id === 'api' || id === 'auth') {
  ...
  async verifyUser(id, config) {
    if (id === 'core' && !config.provided.userSecret) {
      ...
      await this.checkUser(name, config, grantAuthPerms)
    }
  }
  ...
  async checkUser(id, config, grantAuthPerms) {
    ...
    const user_key = randtoken.uid(64)
    const user_secret = randtoken.uid(64)
    ...
    const user = {
      ...
      refresh_token: user_key + randtoken.uid(64)
    }
